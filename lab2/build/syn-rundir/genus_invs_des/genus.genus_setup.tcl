...
################################################################################
if {[::legacy::find -design design:fir] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:fir
}
...
# slack
set _slk_ [::legacy::get_attribute slack design:fir]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "2835.3"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: 2835.3,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
if {"[string_representation [::legacy::get_attribute slack_by_mode design:fir]]" != "{{mode:fir/ss_100C_1v60.setup_view 2835.3}}"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack_by_mode: {{mode:fir/ss_100C_1v60.setup_view 2835.3}}  current slack_by_mode: [string_representation [::legacy::get_attribute slack_by_mode design:fir]]"
}
# tns
set _tns_ [::legacy::get_attribute tns design:fir]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:fir]
...
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:fir]
if {[regexp {^-?[0-9.]+$} $_net_area_]} {
  set _net_area_ [format %.0f $_net_area_]
}
if {$_net_area_ != "431"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden net area: 431,  current net area: $_net_area_"
}
unset _net_area_
